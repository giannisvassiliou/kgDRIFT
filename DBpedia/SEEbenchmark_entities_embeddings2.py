...
Example
-------
python benchmark_all_algorithms_single_pass.py --input-files TESToutAI1.txt TESToutAI2.txt TESToutAI3.txt TESToutAI4.txt TESToutAI5.txt TESToutAI6.txt TESToutAI7.txt --outdir benchmark_results
"""
...
ALGORITHMS = ["agglomerative", "spectral", "gmm", "kmeans", "dbscan"]
...
def read_two_columns(path: str, sep: Optional[str] = None) -> List[Tuple[str, str]]:
    ...
    for ln in lines:
        ...
        if len(parts) >= 2:
            ...
            # 🔥 NEW: handle single column
            entity = ln.strip()
            label = entity  # duplicate it
...
def process_algorithm_k(
    ...
    for month in month_inputs:
        ...
        if n_effective >= 2:
            try:
                ...
                sil = float(silhouette_score(month.emb_unit[mask], cluster_labels[mask], metric="cosine"))
            except Exception:
                sil = float("nan")
            ...
            rows_sum.append(
                {
                    ...
                    if not np.isnan(coherences[i]) else np.nan,
        ...
        print(f"      silhouette={sil:.4f}  pair_cosine={coherence_weighted:.4f}  effective_clusters={n_effective}")
    ...
    if args.plot and not pair_df.empty:
        ...
        plt.title(f"{algorithm.upper()} Interest Drift, k={k}")
        plt.tight_layout()
        ...
        plt.xlabel("Consecutive month pair")
        plt.title(f"{algorithm.upper()} Semantic Drift, k={k}")
        plt.tight_layout()
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="Monthly input files. Each should be a 2-column file: entity, label.",
    )
    parser.add_argument(
        "--outdir",
    ...
    parser.add_argument(
        "--plot",
        action="store_true",
        help="Generate plots for each algorithm/k run.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="SpectralClustering n_neighbors.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--kmeans-n-init",
        default="auto",
        help="KMeans n_init. Use integer or 'auto'.",
    )
    parser.add_argument(
        "--kmeans-max-iter",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.01,
        help="DBSCAN eps minimum.",
    )
    parser.add_argument(
        "--dbscan-eps-max",
        type=float,
        default=1.00,
        help="DBSCAN eps maximum.",
    )
    parser.add_argument(
        "--dbscan-eps-steps",
        type=int,
        default=100,
        help="DBSCAN eps search steps.",
    )
    parser.add_argument(
        "--dbscan-include-noise-as-cluster",
        action="store_true",
        help="Treat DBSCAN noise as its own cluster.",
    )
    ...
    for path in args.input_files:
        ...
        print(f"  [{mname}] Encoding {len(entities)} entities (NOT labels)...")
        sbert_texts = [ent.replace("_", " ").title() for ent in entities]
    ...
    for algorithm in args.algorithms:
        for k in args.k_values:
            ...
            mean_pair_cosine = month_quality_df[
                "weighted_mean_intra_cluster_pairwise_cosine_similarity"
            ].mean()
    ...
    all_month_df = (
        pd.concat(all_month_metrics, ignore_index=True)
        if all_month_metrics
        else pd.DataFrame()
    )
